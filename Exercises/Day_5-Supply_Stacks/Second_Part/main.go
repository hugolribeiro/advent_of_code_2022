...
func doTheMovement(moves, from, to int) {
	for movement := moves; movement > 0; movement-- {
		startingStacks[to] = append(startingStacks[to], startingStacks[from][len(startingStacks[from])-movement])

	}
	startingStacks[from] = startingStacks[from][0 : len(startingStacks[from])-moves]
}
