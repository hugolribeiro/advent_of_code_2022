...
func getTotalAssignmentPairs(inputFile *os.File) int {
	...
	for scanner.Scan() {
		...
		if firstAssignedFirstNumber <= secondAssignedFirstNumber && firstAssignedSecondNumber >= secondAssignedSecondNumber {
			totalAssignmentPairs += 1
			continue
		}

		if secondAssignedFirstNumber <= firstAssignedFirstNumber && secondAssignedSecondNumber >= firstAssignedSecondNumber {
			totalAssignmentPairs += 1
		}
	}
